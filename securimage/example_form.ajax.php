...
<div id="success_message" style="display: none">Deine Nachricht wurde gesendet!<br />Wir werden uns so schnell wie möglich bei Dir melden.</div>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && @$_POST['do'] == 'contact') {
        ...
        if (isset($GLOBALS['DEBUG_MODE']) && $GLOBALS['DEBUG_MODE'] == false) {
            ...
            if (strlen($name) < 3) {
                ...
                $errors['name_error'] = 'Dein Name ist erforderlich';
            }

            if (strlen($email) == 0) {
                // no email address given
                $errors['email_error'] = 'Email Addresse ist erforderlich';
            } else if ( !preg_match('/^(?:[\w\d]+\.?)+@(?:(?:[\w\d]\-?)+\.)+\w{2,4}$/i', $email)) {
                // invalid email format
                $errors['email_error'] = 'Email Addresse entered ist erforderlich';
            }

            if (strlen($message) < 20) {
                // message length too short
                $errors['message_error'] = 'Bitte gib eine Nachricht ein';
            }
        }
        ...
        if (sizeof($errors) == 0) {
            ...
            $time       = date('r');
            $message = "Über das Kontaktformular wurde eine Nachricht gesendet. Die folgenden Informationen wurden bereitgestellt.<br /><br />"
                     . "Name: $name<br />"
                     . "Email: $email<br />"
                     . "URL: $URL<br />"
                     . "Message:<br />"
