...
function get_the_qa_menu() {
	...
	if ( ($user_ID == 0 && qa_visitor_can( 'publish_questions' )) || current_user_can( 'publish_questions' ) ) {
		$menu[] = array(
			'title'		 => __( 'Stelle eine Frage', QA_TEXTDOMAIN ),
			'type'		 => 'ask',
...
function get_the_qa_error_notice() {
	...
	$out .= __( 'Bei der Verarbeitung Deiner Übermittlung ist ein Fehler aufgetreten.', QA_TEXTDOMAIN );
	$out .= '</div>';
}
...
function get_the_qa_search_form() {
	...
	$out .= '<button>' . __( 'Suche', QA_TEXTDOMAIN ) . '</button>';
...
function get_the_qa_pagination( $query = null ) {
	...
	if ( $current_page > 1 )
		$out .= get_qa_single_page_link( $query, $current_page - 1, __( 'vorher', QA_TEXTDOMAIN ), 'prev' );
	...
	if ( $current_page < $total_pages )
		$out .= get_qa_single_page_link( $query, $current_page + 1, __( 'nächste', QA_TEXTDOMAIN ), 'next' );
...
function get_the_qa_time( $id ) {
	...
	if ( $time_diff > 0 && $time_diff < 24 * 60 * 60 )
		$h_time	 = sprintf( __( '%s zuvor', QA_TEXTDOMAIN ), human_time_diff( $time ) );
	else
...
function get_the_qa_action_links( $id ) {
	...
	if ( current_user_can( 'edit_post', $id ) )
		$links[ 'edit' ] = __( 'bearbeiten', QA_TEXTDOMAIN );

	if ( current_user_can( 'delete_post', $id ) )
		$links[ 'delete' ] = __( 'löschen', QA_TEXTDOMAIN );

	if ( is_user_logged_in() ) {
		if ( current_user_can( 'flag_questions', $id ) )
			$links[ 'flag' ] = __( 'melden', QA_TEXTDOMAIN );
	}
	...
	foreach ( $links as $type => $title ) {
		if ( 'flag' == $type ) {
			...
			if ( isset( $_GET[ 'flag_received' ] ) )
				$links[ 'flag' ] = '<span style="color:green">' . __( 'Dein Bericht wurde erhalten.', QA_TEXTDOMAIN ) . '</span>';
			else if ( isset( $_GET[ 'no_reason' ] ) ) {
				$links[ 'flag' ] = $flag_link . " " . '<span style="color:red">' . __( 'Bitte wähle einen Grund für die Berichterstattung.', QA_TEXTDOMAIN ) . '</span>';
				$show_form		 = true;
			} else if ( isset( $_GET[ 'flag_error' ] ) ) {
				$links[ 'flag' ] = $flag_link . " " . '<span style="color:red">' . __( 'CAPTCHA Fehler. Bitte versuche es erneut.', QA_TEXTDOMAIN ) . '</span>';
				$show_form		 = true;
			} else {
...
function the_qa_flag_form( $id ) {
	...
	if ( isset( $qa_general_settings[ "report_reasons" ] ) && '' != trim( $qa_general_settings[ "report_reasons" ] ) ) {
		...
		if ( is_array( $reasons ) ) {
			...
			$f .= __( 'Wähle einen Grund für die Berichterstattung aus:', QA_TEXTDOMAIN );
			$f .= '<br />';
	...
	if ( isset( $qa_general_settings[ "captcha" ] ) && $qa_general_settings[ "captcha" ] && qa_is_captcha_usable() ) {
		...
		$f .= sprintf( '<br/><label class="qa_captcha"><img src="%s" style="vertical-align:top" /> <input type="text" name="random" placeholder="%s"/></label> ', QA_PLUGIN_URL . 'default-templates/captcha-image.php', __( 'Zeichen eingeben', QA_TEXTDOMAIN ) );
...
//		</div>';
	...
	$f .= '<input type="submit" value="' . __( 'Bericht senden', QA_TEXTDOMAIN ) . '" />';
	$f .= '</form>';
	//$f .= '<br />';
	$f .= '<input type="submit" value="' . __( 'Abbrechen', QA_TEXTDOMAIN ) . '" onClick="javascript:document.getElementById(\'qa_flag_form_' . $id . '\').style.display=\'none\';" />';
	$f .= '</div>';
...
function get_the_question_score( $question_id = 0, $label = true, $count_class = 'mini-count' ) {
	...
	$out .= "<div class='" . $count_class . "'>" . __( 'Stimmen:', QA_TEXTDOMAIN ) . ' ' . number_format_i18n( $score ) . "</div>";
...
function get_the_question_voting( $question_id = 0 ) {
	...
	$buttons = array(
		'up'	 => __( 'Diese Frage ist nützlich und klar (zum Rückgängigmachen erneut klicken)', QA_TEXTDOMAIN ),
		'down'	 => __( 'Diese Frage ist unklar oder nicht nützlich (zum Rückgängigmachen erneut klicken)', QA_TEXTDOMAIN )
	);
	...
	$out .= $buttons[ 'up' ];
	$out .= '<span title="' . __( 'Ergebnis', QA_TEXTDOMAIN ) . '">' . number_format_i18n( $up - $down ) . '</span>';
	$out .= $buttons[ 'down' ];
	$out .= '</div>';
...
function get_the_question_subscription() {
	...
	get_queried_object_id(), __( 'Klicke hier, um per E-Mail über weitere Antworten informiert zu werden', QA_TEXTDOMAIN ), __( 'Hör auf, mich per E-Mail über Folgeantworten zu informieren', QA_TEXTDOMAIN )
	);
}
...
function get_the_answer_voting( $answer_id ) {
	...
	$buttons = array(
		'up'	 => __( 'Diese Antwort ist nützlich (erneut klicken, um rückgängig zu machen)', QA_TEXTDOMAIN ),
		'down'	 => __( 'Diese Antwort ist nicht hilfreich (erneut klicken, um rückgängig zu machen)', QA_TEXTDOMAIN )
	);
...
function get_the_answer_accepted( $answer_id ) {
	...
	if ( $user_can_accept ) {
		...
		$out .= _qa_html( 'input', array(
			'type'	 => 'submit',
			'title'	 => __( 'Antwort akzeptieren (erneut klicken, um rückgängig zu machen)', QA_TEXTDOMAIN ),
			'class'	 => 'vote-accepted-' . ( $is_accepted ? 'on' : 'off' )
		) );
		$out .= '</form>';
	} elseif ( $is_accepted ) {
		$out .= _qa_html( 'span', array(
			'title'	 => __( 'Akzeptierte Antwort', QA_TEXTDOMAIN ),
			'class'	 => 'vote-accepted-on'
		), __( 'akzeptiert', QA_TEXTDOMAIN ) );
	}
	return $out;
}
...
function get_the_question_status( $question_id = 0, $label = true, $count_class = 'mini-count' ) {
	...
	$out .= "<div class='" . $count_class . "'>" . __( 'Antworten:', QA_TEXTDOMAIN ) . ' ' . number_format_i18n( $count ) . "</div>";
...
function get_the_question_form() {
	...
	$out .= '<input type="text" id="question-title" name="question_title" placeholder="' . esc_attr( __( 'Fragetitel', QA_TEXTDOMAIN ) ) . '" value="' . esc_attr( $question->post_title ) . '" />';
	$out .= '</div></div>';
	...
	$out .= wp_dropdown_categories( array(
		...
		'name'				 => 'question_cat',
		'class'				 => '',
		'show_option_none'	 => __( 'Kategorie wählen...', QA_TEXTDOMAIN ),
		'echo'				 => 0
	) );
	$out .= '</div>
	...
	if ( $qa_general_settings[ 'captcha' ] ) {
		$out .= sprintf( '<label><img src="%s" style="vertical-align:top" /> <input type="text" name="random" placeholder="%s"/></label> ', QA_PLUGIN_URL . 'default-templates/captcha-image.php', __( 'Zeichen eingeben', QA_TEXTDOMAIN ) );
	}
...
function get_the_answer_count( $question_id = 0 ) {
	...
	return sprintf( _n( '1 Antwort', '%d Antworten', $count, QA_TEXTDOMAIN ), number_format_i18n( $count ) );
}
...
function get_the_answer_form() {
	...
	if ( isset( $wp->query_vars[ 'qa_edit' ] ) ) {
		...
		if ( !current_user_can( 'publish_answers' ) ) {
			$out .= '<p>' . __( 'Du darfst keine Antworten hinzufügen!', QA_TEXTDOMAIN ) . '</p>';
			return;
		}
		$answer = (object) array(
			'ID'			 => '',
	...
	$out .= '<form id="qa-answer-form" method="post" action="' . qa_get_url( 'archive' ) . '">';
...
function get_the_qa_submit_button() {
	...
	/* && qa_visitor_can( 'immediately_publish_questions' ) */ ) ) {
		$button = __( 'Einreichen', QA_TEXTDOMAIN );
	} elseif ( get_option( 'users_can_register' ) ) {
		$button = __( 'Registrieren/Anmelden und Senden', QA_TEXTDOMAIN );
	} else {
		$button = __( 'Anmelden und senden', QA_TEXTDOMAIN );
	}
