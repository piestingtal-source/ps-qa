...
class QA_Edit {
	...
	*/
	...
	function handle_question_editing() {
		...
		wp_die( __( 'Nonce-Fehler: Du hast anscheinend keine Berechtigung dazu.', QA_TEXTDOMAIN ) );
		...
		if( $qa_general_settings['captcha'] ) {
			...
			wp_die( __( 'Die eingegebenen Zeichen stimmen nicht mit dem Bild überein. Bitte verwende die Zurück-Schaltfläche Deines Browsers, um Deine Frage zu bearbeiten.', QA_TEXTDOMAIN ) );
		}
		...
		wp_die( __( 'Fragen müssen sowohl einen Titel als auch einen Text haben. Bitte verwende die Zurück-Schaltfläche Deines Browsers, um Deine Frage zu bearbeiten.', QA_TEXTDOMAIN ) );
	...
	function handle_answer_editing() {
		...
		wp_die( __( 'Die Antwort muss mit einer Frage verknüpft sein.', QA_TEXTDOMAIN ) );

		if ( empty( $answer['post_content'] ) )
		wp_die( __( 'Du musst tatsächlich etwas schreiben. Bitte verwende die Zurück-Schaltfläche Deines Browsers, um Deine Antwort zu bearbeiten.', QA_TEXTDOMAIN ) );
		...
		if ( $dup_id ) {
			wp_die( sprintf( __( 'Es scheint, dass die Antwort <a href="%s">bereits gegeben wurde</a>.', QA_TEXTDOMAIN ), qa_get_url( 'single', $dup_id ) ) );
		}
	...
	function _insert_post( $post_id, $post_args, $defaults ) {
		if ( !$post_id ) {
			...
			wp_die( __( 'Du postest
			 zu schnell. Langsamer.', QA_TEXTDOMAIN ) );
	...
	function login_message( $msg ) {
		...
		$text = sprintf( __( 'Um die Veröffentlichung Deiner %s abzuschließen, registriere Dich bitte unten. Wenn Du bereits ein Konto hast, melde Dich stattdessen <a href="%s">an</a>.', QA_TEXTDOMAIN ), get_post_type( $post_id ), wp_login_url() );
	...
	function transition_post_status( $new_status, $old_status, $post ) {
		...
		if ( 'answer' == $post->post_type && 'publish' == $new_status ) {
			...
			if (function_exists('bp_activity_add')) {
				...
				$action	= sprintf(__('%1$s beantwortete Frage "<a href="%2$s">%3$s</a>"', 'qa'), $current_user->display_name, get_permalink($post->ID), $question->post_title);
		...
		if ( 'question' == $post->post_type && 'publish' == $new_status ) {
			...
			if (function_exists('bp_activity_add')) {
				...
				$action = sprintf(__('%1$s fragte "<a href="%2$s">%3$s</a>"', 'qa'), $current_user->display_name, get_permalink($post->ID), $post->post_title);
				$action = apply_filters('qa_bp_activity_action', $action, $post );
