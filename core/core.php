...
class QA_Core {
	...
	function parse_request( $wp ) {
		...
		if ( isset( $wp->query_vars['qa_edit'] ) ) {
			...
			if ( $theme_file != '' ) {
				...
				$args = array(
					'slug'        => $wp->request,
					'title'       => __( 'Antwort bearbeiten', QA_TEXTDOMAIN ),
		...
		if ( isset( $wp->query_vars['qa_ask'] ) ) {
			...
			if ( $theme_file != '' ) {
				...
				$args = array(
					'slug'        => $wp->request,
					'title'       => __( 'Frage stellen', QA_TEXTDOMAIN ),
		...
		if ( isset( $wp->query_vars['author_name'] ) && isset( $wp->query_vars['post_type'] ) && $wp->query_vars['post_type'] == 'question' ) {
			$args = array(
				'slug'          => $wp->request,
				'title'         => __( 'Benutzerprofil', QA_TEXTDOMAIN ),
	...
	function qa_flag() {
		...
		if ( is_user_logged_in() ) {
			...
			$new_meta["user"] = __( 'Unbekannt', QA_TEXTDOMAIN );
		}

		if ( isset( $_POST["report_reason"] ) ) {
			$new_meta["reason"] = $_POST["report_reason"];
		} else {
			$new_meta["reason"] = __( 'Keiner', QA_TEXTDOMAIN );
		}
		...
		if ( isset( $this->g_settings["report_email"] ) && is_email( $this->g_settings["report_email"] ) && $new_meta["count"] == 1 ) {
			...
			$subject = __( 'Eine Frage oder Antwort wurde gemeldet', QA_TEXTDOMAIN );
			$message = __( 'Gemeldet von:', QA_TEXTDOMAIN );
			$message .= $new_meta["user"];
			$message .= "\n";
			$message .= __( 'Meldegrund:', QA_TEXTDOMAIN );
			$message .= stripslashes( $new_meta["reason"] );
			$message .= "\n\n";
			$message .= __( 'Du kannst es bearbeiten, indem Du auf diesen Link klickst:', QA_TEXTDOMAIN );
			$message .= "\n\n";
	...
	function init() {
		...
		register_taxonomy( 'question_category', 'question', array(
			...
			'labels'       => array(
				'name'              => __( 'Fragenkategorien', QA_TEXTDOMAIN ),
				'singular_name'     => __( 'Fragenkategorie', QA_TEXTDOMAIN ),
				'search_items'      => __( 'Fragenkategorien suchen', QA_TEXTDOMAIN ),
				'all_items'         => __( 'Alle Fragenkategorien', QA_TEXTDOMAIN ),
				'parent_item'       => __( 'Übergeordnete Fragekategorie', QA_TEXTDOMAIN ),
				'parent_item_colon' => __( 'Übergeordnete Frage Kategorie:', QA_TEXTDOMAIN ),
				'edit_item'         => __( 'Fragenkategorie bearbeiten', QA_TEXTDOMAIN ),
				'update_item'       => __( 'Fragenkategorie aktualisieren', QA_TEXTDOMAIN ),
				'add_new_item'      => __( 'Neue Fragenkategorie hinzufügen', QA_TEXTDOMAIN ),
				'new_item_name'     => __( 'Name der neuen Fragekategorie', QA_TEXTDOMAIN ),
			)
		) );
		...
		register_taxonomy( 'question_tag', 'question', array(
			...
			'labels'       => array(
				'name'                       => __( 'Frage-Tags', QA_TEXTDOMAIN ),
				'singular_name'              => __( 'Frage-Tag', QA_TEXTDOMAIN ),
				'search_items'               => __( 'Frage-Tags suchen', QA_TEXTDOMAIN ),
				'popular_items'              => __( 'Beliebte Frage-Tags', QA_TEXTDOMAIN ),
				'all_items'                  => __( 'Alle Frage-Tags', QA_TEXTDOMAIN ),
				'edit_item'                  => __( 'Frage-Tag bearbeiten', QA_TEXTDOMAIN ),
				'update_item'                => __( 'Frage-Tag aktualisieren', QA_TEXTDOMAIN ),
				'add_new_item'               => __( 'Neues Frage-Tag hinzufügen', QA_TEXTDOMAIN ),
				'new_item_name'              => __( 'Neuer Name des Frage-Tags', QA_TEXTDOMAIN ),
				'separate_items_with_commas' => __( 'Trenne Frage-Tags durch Kommas', QA_TEXTDOMAIN ),
				'add_or_remove_items'        => __( 'Hinzufügen oder Entfernen von Frage-Tags', QA_TEXTDOMAIN ),
				'choose_from_most_used'      => __( 'Wähle aus den am häufigsten verwendeten Frage-Tags', QA_TEXTDOMAIN ),
			)
		) );
		...
		$args = array(
			...
			'labels'          => array(
				'name'               => __( 'Fragen', QA_TEXTDOMAIN ),
				'singular_name'      => __( 'Frage', QA_TEXTDOMAIN ),
				'add_new'            => __( 'Neue Frage', QA_TEXTDOMAIN ),
				'add_new_item'       => __( 'Neue Frage hinzufügen', QA_TEXTDOMAIN ),
				'edit_item'          => __( 'Frage bearbeiten', QA_TEXTDOMAIN ),
				'new_item'           => __( 'Neue Frage', QA_TEXTDOMAIN ),
				'view_item'          => __( 'Frage anzeigen', QA_TEXTDOMAIN ),
				'search_items'       => __( 'Fragen suchen', QA_TEXTDOMAIN ),
				'not_found'          => __( 'Keine Fragen gefunden.', QA_TEXTDOMAIN ),
				'not_found_in_trash' => __( 'Keine Fragen im Papierkorb gefunden.', QA_TEXTDOMAIN ),
			)
		);
	...
	function load_default_style() {
		...
		if ( ! current_theme_supports( 'qa_script' ) ) {
			...
			wp_localize_script( 'qa-init', 'QA_L10N', array(
				...
				'msg_login'     => __( 'Bitte logge Dich ein oder registriere Dich, um abzustimmen.', QA_TEXTDOMAIN ),
				'msg_own'       => __( 'Du kannst nicht über Deinen eigenen Beitrag abstimmen.', QA_TEXTDOMAIN ),
	...
	function wp_title( $title, $sep, $seplocation ) {
		...
		if ( is_qa_page( 'ask' ) ) {
			$new_title = __( 'Stelle eine Frage', QA_TEXTDOMAIN );
			...
			if ( ! is_object( $bp ) ) {
				$new_title = sprintf( __( 'Benutzer: %s', QA_TEXTDOMAIN ), $user->display_name );
			}
		}
