...
class QA_Core_Admin extends QA_Core {
	...
	function __construct() {
		...
		$this->capability_map = array(
			'read_questions'                => __( 'Fragen anzeigen', QA_TEXTDOMAIN ),
			'publish_questions'             => __( 'Fragen stellen', QA_TEXTDOMAIN ),
			'immediately_publish_questions' => __( 'Veröffentliche Fragen und Antworten sofort (andernfalls werden sie als ausstehend gespeichert).', QA_TEXTDOMAIN ),
			'edit_published_questions'      => __( 'Eigene Fragen bearbeiten', QA_TEXTDOMAIN ),
			'delete_published_questions'    => __( 'Lösche eigene Fragen', QA_TEXTDOMAIN ),
			'edit_others_questions'         => __( 'Bearbeite die Fragen anderer', QA_TEXTDOMAIN ),
			'delete_others_questions'       => __( 'Lösche andere Fragen', QA_TEXTDOMAIN ),
			'subscribe_to_new_questions'    => __( 'Abonniere neue Fragen', QA_TEXTDOMAIN ),

			'read_answers'             => __( 'Antworten anzeigen', QA_TEXTDOMAIN ),
			'publish_answers'          => __( 'Antworten hinzufügen', QA_TEXTDOMAIN ),
			'edit_published_answers'   => __( 'Bearbeite eigene Antworten', QA_TEXTDOMAIN ),
			'delete_published_answers' => __( 'Löscht eigene Antworten', QA_TEXTDOMAIN ),
			'edit_others_answers'      => __( 'Bearbeite die Antworten anderer', QA_TEXTDOMAIN ),
			'delete_others_answers'    => __( 'Lösche die Antworten anderer', QA_TEXTDOMAIN ),

			'flag_questions' => __( 'Fragen und Antworten melden', QA_TEXTDOMAIN ),
			'upload_files'   => __( 'Dateien hochladen (Notwendig für das Hochladen von Medien, NICHT an QA-Besucher weitergeben)', QA_TEXTDOMAIN ),
		);
	...
	function add_metabox() {
		...
		if ( $result != null ) {
			if ( 'question' == $post->post_type ) {
				add_meta_box(
					'reported_question',
					__( 'Diese Frage wurde gemeldet', QA_TEXTDOMAIN ),
				...
				add_meta_box(
					'reported_answer',
					__( 'Diese Antwort wurde gemeldet', QA_TEXTDOMAIN ),
					array( &$this, 'print_metabox' ),
					'answer',
					'side',
					'high'
	...
	function print_metabox() {
		...
		echo '<label>';
		_e( 'Anzahl der Meldungen:', QA_TEXTDOMAIN );
		echo '</label> ';
		...
		echo '<label>';
		_e( 'Zuletzt gemeldet von:', QA_TEXTDOMAIN );
		echo '</label> ';
		echo '<span class="qa_reported">';
		if ( isset( $meta["user"] ) ) {
			echo $meta["user"];
		...
		echo '<label>';
		_e( 'Grund der letzten Meldung:', QA_TEXTDOMAIN );
		echo '</label> ';
		echo '<span class="qa_reported">';
		...
		echo '<label>';
		_e( 'Meldung löschen:', QA_TEXTDOMAIN );
		echo '</label> ';
		echo '<span class="qa_reported">';
	...
	function add_column( $columns ) {
		...
		$columns = array(
			...
			'title'       => __( 'Titel', QA_TEXTDOMAIN ),
			'qa-question' => __( 'Frage', QA_TEXTDOMAIN ),
			'author'      => __( 'Autor', QA_TEXTDOMAIN ),
			'date'        => __( 'Datum', QA_TEXTDOMAIN ),
			'qa-reported' => __( 'Gemeldet', QA_TEXTDOMAIN ),
		);
	...
	function show_column( $name ) {
		...
		switch ( $name ) {
			case 'qa-reported': {
				$meta = get_post_meta( $post->ID, '_qa_report', true );
				if ( $meta ) {
					echo '<span style="color:red;font-weight:bold">' . __( 'Ja', QA_TEXTDOMAIN ) . '</span>';
				} else {
					echo '<span style="color:green;font-weight:bold">' . __( 'Nein', QA_TEXTDOMAIN ) . '</span>';
				}
				break;
			}
			case 'qa-question': {
				...
				printf( '<a href="%s" title="%s" target="question" ><strong>%s</strong></a><p>%s</p>',
					get_permalink(),
					__( 'Zur Fragenseite gehen', QA_TEXTDOMAIN ),
					get_the_title(),
					get_the_excerpt() );
	...
	function add_question_counts() {
		...
		$text      = _n( 'Frage veröffentlicht', 'Fragen veröffentlicht', intval( $num_posts->publish ) );
		...
		if ( $num_posts->pending > 0 ) {
			...
			$text = _n( 'Frage ausstehend', 'Fragen ausstehend', intval( $num_posts->pending ) );
			if ( current_user_can( 'edit_posts' ) ) {
		...
		$num       = number_format_i18n( $num_posts->publish );
		$text      = _n( 'Antwort veröffentlicht', 'Antworten veröffentlicht', intval( $num_posts->publish ) );
		if ( current_user_can( 'edit_posts' ) ) {
		...
		if ( $num_posts->pending > 0 ) {
			$num  = number_format_i18n( $num_posts->pending );
			$text = _n( 'Antwort ausstehend', 'Antworten ausstehend', intval( $num_posts->pending ) );
			if ( current_user_can( 'edit_posts' ) ) {
		...
		if ( $count > 0 ) {
			...
			$text = _n( 'Q&A Gemeldet', 'Q&A Gemeldet', intval( $count ) );
			if ( current_user_can( 'edit_posts' ) ) {
	...
	function notice_settings() {
		...
		if ( ! $no_visit && $screen->id != $this->page ) {
			...
			echo '<div class="updated fade"><p>' .
			     sprintf( __( "<b>[Q&A]</b> Es sieht so aus, als hättest Du gerade PS QA installiert oder aktualisiert. Möglicherweise möchtest Du einige %s anpassen. <b>Beachte beim Upgrade, dass die Handhabung des Seitenlayouts geändert wurde.</b>.", QA_TEXTDOMAIN ), "<a href='" . admin_url( 'edit.php?post_type=question&page=qa_settings' ) . "'>" . __( "Einstellungen", QA_TEXTDOMAIN ) . "</a>" ) .
			     '</p></div>';
		}
		...
		if ( 'assign' != @$qa_general_settings["method"] && ! get_option( 'users_can_register' ) ) /* translators: %s means settings here */ {
			echo '<div class="error fade"><p>' .
			     sprintf( __( "<b>[Q&A]</b> <i>Nachdem der Besucher eine Frage gestellt hat</i> ist die Registrierung des Besuchers erforderlich, Deine Website ist jedoch für Registrierungen geschlossen. Du kannst dies mithilfe des Plugins %s oder der Wordpress-Einstellungen beheben.", QA_TEXTDOMAIN ), "<a href='" . admin_url( 'edit.php?post_type=question&page=qa_settings' ) . "'>" . __( "Einstellungen", QA_TEXTDOMAIN ) . "</a>" ) .
			     '</p></div>';
		}
		...
		if ( ! get_role( 'visitor' ) ) {
			echo '<div class="error fade"><p>' .
			     __( "<b>[Q&A]</b>Besucherrolle kann nicht gefunden werden. Versuche das Plugin zu deaktivieren und wieder zu aktivieren. Wenn diese Meldung weiterhin angezeigt wird, wende Dich an unsere Hilfe und den Support.", QA_TEXTDOMAIN ) .
			     '</p></div>';
		}
		...
		if ( ! get_option( 'permalink_structure' ) ) {
			echo '<div class="error fade"><p>' .
			     __( "<b>[Q&A]</b> Das Plugin funktioniert mit der Standard-Permalink-Struktur nicht richtig. Du musst eine hübsche Permalink-Struktur verwenden.", QA_TEXTDOMAIN ) .
			     '</p></div>';
		}
	...
	function set_plugin_meta( $links, $file ) {
		...
		if ( $file == $plugin ) {
			return array_merge(
				$links,
				array( sprintf( '<a href="admin.php?page=%s">%s</a>', $this->plugin_name, __( 'Einstellungen' ) ) )
			);
		}
	...
	function init_defaults() {
		...
		if ( ! $visitor_role ) {
			add_role( 'visitor', 'QA Besucher', array(
				'read_questions'                => true,
	 ...
	 */
	function admin_menu() {
		$this->hook_suffix = add_submenu_page( 'edit.php?post_type=question', __( 'Einstellungen', QA_TEXTDOMAIN ), __( 'Einstellungen', QA_TEXTDOMAIN ), 'manage_options', 'qa_settings', array(
			&$this,
	...
	function estimate() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			die( json_encode( array( 'error' => esc_js( __( 'Du bist dazu nicht berechtigt', QA_TEXTDOMAIN ) ) ) ) );
		}

		$c_theme = get_template(); // Current theme

		if ( strpos( qa_supported_themes(), $c_theme ) !== false ) {
			die( json_encode( array( 'error' => sprintf( __( 'Dein aktuelles Theme %s wird bereits von PS QA unterstützt. Es sind keine zusätzlichen CSS-Regeln erforderlich.', QA_TEXTDOMAIN ), $c_theme ) ) ) );
		}
		...
		if ( ! isset( $_POST['page_layout'] ) || ! isset( $_POST['page_width'] ) || ! isset( $_POST['content_width'] ) || ! isset( $_POST['sidebar_width'] ) || ! isset( $_POST['content_alignment'] ) ) {
			die( json_encode( array( 'error' => esc_js( __( 'Mindestens eine der Variablen fehlt', QA_TEXTDOMAIN ) ) ) ) );
		}
		...
		switch ( $page_layout ) {
			...
			default:
				die( json_encode( array( 'error' => esc_js( __( 'Seitenlayout ist nicht ausgewählt', QA_TEXTDOMAIN ) ) ) ) );
				break;
		}
		...
		if ( $css ) {
			...
			if ( update_option( QA_OPTIONS_NAME, $qa_general_settings ) ) {
				...
				die( json_encode( array( 'error' => esc_js( __( 'Nichts wurde geändert. Bitte überprüfe Deine Einstellungen.', QA_TEXTDOMAIN ) ) ) ) );
			}
		}
