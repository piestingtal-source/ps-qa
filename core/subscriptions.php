...
class QA_Subscriptions {
	...
	function notify( $new_status, $old_status, $post ) {
		...
		$subject = sprintf( __( '[%s] Neue Antwort auf "%s"' ), get_option( 'blogname' ), $question->post_title );

		$content = sprintf( __( '%s hat eine neue Antwort auf %s hinzugefügt:', QA_TEXTDOMAIN ),
		...
		foreach ( $subscribers as $subscriber_id ) {
			...
			if ( $post->post_author != $subscriber_id ) {
				$msg = $content . sprintf( __( 'Um Dein Abonnement zu verwalten, besuche <a href="%s">die Frage</a>.', QA_TEXTDOMAIN ), qa_get_url( 'single', $post->ID ) );
			} else {
				$msg = $content;
			}
