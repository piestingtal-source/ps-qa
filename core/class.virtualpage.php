...
if ( !class_exists( 'QA_Virtual_Page' ) ) {
	...
	class QA_Virtual_Page {
		...
		function __construct( $args ) {
			if ( !isset( $args[ 'slug' ] ) )
				throw new Exception( 'Für die virtuelle Seite wurde kein Slug angegeben' );
			...
			$this->post_type		 = isset( $args[ 'post_type' ] ) ? $args[ 'post_type' ] : 'public';
			$this->max_num_pages	 = isset( $args[ 'max_num_pages' ] ) ? $args[ 'max_num_pages' ] : '';
		...
		function virtualPage( $posts ) {
			...
			if ( $old_post_slug_id == '' ) {
				...
				$post->ID					 = $this->ID; /* 99999999999 non-existed post ID */
				...
				if ( isset( $this->max_num_pages ) && is_numeric( $this->max_num_pages ) ) {
